...
export class ContatoHttpService {
  private apiUrl = environment.apiUrl;;

  constructor(private http: HttpClient) { }

  getAll(clienteId: string | null){
    return this.http.get<Contato[]>(this.apiUrl + `/contatos/${clienteId}`);
  }
  ...
  inserir(contato: any): Observable<any> {
    return this.http.post(`${this.apiUrl}/contato`, contato);
  }

  selecionarPorNomeENumero(contato: Contato): Observable<any[]> {
    return this.http.get(this.apiUrl + `/contato/consultarPorNomeENumero/${contato.idcli}/${contato.nome}/${contato.num}`).pipe(
      map((response: any) => response || [])
    );
  }
